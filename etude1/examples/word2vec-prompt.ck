...
fun void print(Word2Vec model) {
    ...
    for (int i; i < model.dim(); i++) {
        cherr <= "dimension " <= i <= " range: [" <= mins[i] <= ", " <= maxs[i] <= "]" <=
            IO.newline();
    }
}
...
fun int execute(string line[]) {
    ...
    if (command == "quit" || command == "exit") {
        ...
        string filepath;
        if (command == "load-50")
            "glove-wiki-gigaword-50.txt" => filepath;
        else if (command == "load-3")
            "glove-wiki-gigaword-50-pca-3.txt" => filepath;
        else
            "glove-wiki-gigaword-50-tsne-2.txt" => filepath;
        ...
        if (line.size() < 5) {
            ...
            <<< "going from", line[1], "to", line[2], "over", line[3], "seconds in", line[4],
               "steps" >>>;
...
fun void go(string start, string end, dur T, int steps, int k) {
    ...
    for (int i; i < vstart.size(); i++) {
        cherr <= vstart[i] <= " ";
    }
    cherr <= IO.newline();
    for (int i; i < vend.size(); i++) {
        cherr <= vend[i] <= " ";
    }
    cherr <= IO.newline();
