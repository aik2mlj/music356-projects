...
chout <= "we'll also turn the frequency of the input way down for the "
             + "occasion.... \n";
chout <= "... \n";
chout.flush();
2::second => now;
...
kf.freq(float) //sets the cutoff frequency.
        This sets both the frequency at which the 2 sample&holds sample the
input signal and the frequency of the sine that crossfades between them.
kf.freq() //gets the cutoff frequency
kf.resonance(float) //sets the resonance, which is implemented as negative
feedback. accepts values between 0 and 0.95 kf.resonance() // gets the
resonance. kf.accent(float) //sets the amount of waveshaping on the crossfading
sine. Ranges from 0 to 1, where 1 is close to regular under-sampling (if no
resonance is used). kf.accent() //gets the accent value.
