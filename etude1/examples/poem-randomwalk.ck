...
true => int shouldScaleTimeToWordLength; // longer words take longer?
...
for (int s; s < NUM_STANZAS; s++) {
    ...
    for (int n; n < LINES_PER_STANZA; n++) {
        ...
        for (int w; w < WORDS_PER_LINE; w++) {
            ...
            // feel free to experiement with what parameters to control with
            // the vector
            Math.remap(vec[0], mins[0], maxs[0], 24, 84) => Std.mtof => bar.freq;
            // use pow to expand/compress the dimensions to affect mapping
            // sensitivity
