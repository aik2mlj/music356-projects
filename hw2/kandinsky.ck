...
DrawEvent drawEvent;
sndBus @=> drawEvent.sndBus; // pass audio bus to drawEvent for shapes
anaBus @=> drawEvent.anaBus;
// polymorphism
Draw @draws[4];
...
ShaderChanger shaderChanger(mouse, drawEvent);
spork ~ shaderChanger.listenToMouse();

// Mosaic synthesizer: analyzes anaBus audio and synthesizes similar samples
Mosaic mosaic;
false => int mosaicEnabled;
// input: pre-extracted model file
if (me.args() > 0) {
    me.arg(0) => string FEATURES_FILE;
    if (mosaic.init(FEATURES_FILE, anaBus)) {
        spork ~ mosaic.run();
        true => mosaicEnabled;
...
// Audio mode switching: m = mosaic only, p = sndBus only, b = both (default)
// Audio mode: 0 = both, 1 = mosaic only, 2 = sndBus only
0 => int audioMode;

<<< "Audio mode keys: M (mosaic only), P (sndBus only), B (both)", "" >>>;
...
while (true) {
    ...
    if (UI.isKeyPressed(UI_Key.M, false)) {
        if (audioMode != 1) {
            1 => audioMode;
            0 => sndBus.gain;           // disconnect sndBus
    ...
    // 'p' key - sndBus only
        ...
        if (audioMode != 2) {
            ...
            1 => sndBus.gain;           // ensure sndBus connected
            <<< "[mode] sndBus only" >>>;
        }
    }
    // 'b' key - both
