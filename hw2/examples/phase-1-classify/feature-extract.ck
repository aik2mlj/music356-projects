...
["blues", "classical", "country", "disco", "hiphop", "jazz", "metal", "pop", "reggae", "rock"] @=> string labels[];
// how many per label
...
// relative path
"/home/aik2/Downloads/gtzan/genres_original/" => string PATH;
...
for (int i; i < labels.size(); i++) {
    ...
    for (int j; j < NUM_EXAMPLES_PER_LABEL; j++) {
        // construct filepath
        PATH + label + "/" + label + ".000" + (j < 10 ? "0" : "") + j + ".wav" => filename;
        // extract the file
